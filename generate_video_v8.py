...
OUTPUT_FILE = r"c:\Users\Manasvi Bhargava\SmartCaneApp\video_assets\draft8_masterpiece.mp4"
...
def add_progress_bar(clip):
    ...
    def draw_bar(get_frame, t):
        ...
        return frame
    if hasattr(clip, 'image_transform'): return clip.image_transform(draw_bar)
    else: return clip.fl_image(draw_bar)
...
def main():
    ...
    # ADD PROGRESS BAR
    final_clip = add_progress_bar(final_clip)
    
    # ADD VOICEOVER
    try:
        vo_path = get_path("aura.mpeg")
        if os.path.exists(vo_path):
            vo_clip = AudioFileClip(vo_path).with_start(0.5)
